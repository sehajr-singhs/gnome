...
# Install transformers without touching torch (T4 needs specific CUDA torch)
try:
    import transformers
except ImportError:
    subprocess.check_call([sys.executable, "-m", "pip", "install", "--quiet", "transformers", "--no-deps"])
    subprocess.check_call([sys.executable, "-m", "pip", "install", "--quiet", "tokenizers", "huggingface_hub", "regex", "safetensors"])
